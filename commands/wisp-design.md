...
**`variantCount` ist VERBINDLICH (Phase 7.18):** Es zählt die DESIGN-VORSCHLÄGE. Poste EXAKT `variantCount` Vorschläge PLUS die Baseline v0 — nicht mehr, nicht weniger (bei `variantCount: 1` also genau 2 Einträge). Der Browser clampt überzählige Einträge hart weg; was du zu viel schickst, sieht niemand. Gesamt-Cap: 8 Einträge (Schema).

**Notification abgeschnitten?** Monitor-Events werden bei langen Payloads (großes `codeSnippet`!) truncated — Felder wie `variantCount`/`deviation`/`sessionId` fehlen dann. NIE raten: Die volle Payload holst du dir per Bridge-Replay `curl -s "http://127.0.0.1:<port>/poll?timeout=500&cursor=0&token=<token>"` (alle Events der Session, JSON) oder aus dem Session-Log `.wisp/sessions/<sessionId>.jsonl` (der configure-Eintrag trägt freeText, codeSnippet, variantCount, deviation).

**1:1 heißt PORTIEREN, nicht nachempfinden:** Enthält das `codeSnippet` vollständiges Markup + CSS (styled-components, Vue-SFC, plain HTML/CSS), dann übersetze es MECHANISCH — Markup: JSX→HTML (`className`→`class`, `{50}`→`"50"`, `defaultChecked`→`checked`); CSS: unverändert aus der Hülle extrahieren (Original-Farben, -Positionen, -Keyframes, -Timings behalten — `@scope` isoliert die Selektoren automatisch, kein Umschreiben nötig). Eine aus dem Gedächtnis gebaute „Annäherung" fällt dem User SOFORT auf (real passiert: vereinfachte Wolken + Glow statt Lichtstrahlen → Beschwerde „sieht nicht 1:1 aus"). Eigene Gestaltung ist nur gefragt, wenn freeText sie verlangt oder das Snippet unvollständig ist.

**`html`-Varianten für 1:1-Treue (Phase 7.18):** Wenn der User per `codeSnippet` eine Referenz mit eigenem MARKUP liefert (SVGs, verschachtelte Struktur), reicht CSS-auf-Klon nicht — gib der Variante zusätzlich ein `html`-Feld (max 30000 Zeichen): vollständiges Ersatz-Markup, das ANSTELLE des Klons im Variant-Host gerendert wird. `css` wirkt weiterhin via `@scope` (`:scope > <wurzel-tag>` adressiert deine Markup-Wurzel) — Styles der Referenz also in `css` packen (mit `!important`), NICHT als `<style>`-Tag ins html (wird gestrippt, genau wie `<script>`, on*-Handler und js/data-URLs; `<svg>` bleibt erhalten). Baseline v0 ignoriert `html` (zeigt immer das Original). Beim Accept kommt das Markup als `variantHtml` im accept-Event zurück — portiere es dann in den ECHTEN Projekt-Stack (Framework-idiomatisch), nie 1:1 als statisches HTML einbauen.
